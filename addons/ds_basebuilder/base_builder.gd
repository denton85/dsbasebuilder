...
const BASE_STRUCTURE = preload("res://addons/ds_basebuilder/components/base_structure.tscn")
...
enum COMPONENT_TYPE_REGISTRY { foundation=0, wall=1, ceiling=2, triangle_foundation=3, triangle_ceiling=4, deconstruct=5, center_connection=6 }
...
## The minimum distance between separate foundation pieces (to make sure you don't build two separate structures on top of each other easily. Maybe needs a rework eventually. The Structure distance is just the distance between the first foundation laid on each stucture.
@export var minimum_structure_distance: float = 20.0
...
func _process(delta: float) -> void:
	if connection_detect.current_focused_connection != null:
		...
		snap.global_rotation.y = connection_detect.current_focused_connection.global_rotation.y + deg_to_rad(snap_rotation)
	else:
		snap.global_position = global_position
		snap.global_rotation = player.global_rotation
		snap.global_rotation.y = player.global_rotation.y + deg_to_rad(snap_rotation)
...
func place_component(component: String, connection: Connection, parent_node: Node3D) -> void:
	...
	if connection.current_components.size() < connection.max_components:
		...
		comp.global_rotation.y += deg_to_rad(snap_rotation)
...
func place_new_structure(component: String, parent_node: Node3D) -> void:
	if DsBbGlobal.total_structures.size() > 0:
		for i in DsBbGlobal.total_structures:
			if i != null and i.global_position.distance_to(snap.global_position) <= minimum_structure_distance:
				return
	if component == null:
		return
	...
	DsBbGlobal.total_structures.append(new_structure)
...
## Set's the build preview to the correct scene.
